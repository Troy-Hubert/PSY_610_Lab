...
ggboxplot(
  ...
  labs(
    subtitle = # we are making a subtitle
...
as.numeric(statistic(results3))



z = as.numeric(statistic(rstatix::wilcox_test(df_long$Depression ~ df_long$Timepoint, paired = TRUE), type="standardized"))
names(z) = "z"
z


rstatix::wilcox_test(df_long$Depression ~ df_long$Timepoint, paired = TRUE)




df  %>%
  rstatix::wilcox_test(depression ~ depression2, paired = TRUE)



length(df$depression2)
length(df$depression)
